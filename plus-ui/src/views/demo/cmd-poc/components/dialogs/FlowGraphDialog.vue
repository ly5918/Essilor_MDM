<template>
  <div v-loading="loading" class="fg-body">
    <template v-if="graph">
      <!-- 场景信息条 -->
      <div class="fg-head">
        <div class="fg-scene">
...
/** 已完成节点数（实例视图的进度口径） */
const doneCount = computed(() => (graph.value?.nodes ?? []).filter(n => n.status === 'COMPLETED').length);
const progressTagType = computed(() => (doneCount.value === (graph.value?.nodes.length ?? 0) ? 'success' : 'warning'));

/** 泳道行：标签 + 行中心 y（泳道无节点时按序号推算，保证 6 行始终呈现） */
const laneRows = computed(() => {
  const lanes = graph.value?.lanes?.length ? graph.value.lanes : DEFAULT_LANES;
