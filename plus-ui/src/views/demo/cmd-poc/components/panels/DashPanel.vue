<template>
  <section class="page dash-page">
    ...
    <template v-if="role.key === 'bu' || role.key === 'gc'">
      <div class="ap-kpis">
        <el-card
          v-for="item in apKpis"
          :key="item.label"
          class="ap-kpi"
          shadow="never"
      ...
      <div class="grid2">
        ...
        <el-card class="page-card" shadow="never" :body-style="{ padding: '0' }">
          <template #header>
            <span class="card-title">{{ role.key === 'gc' ? '高优先级治理决策' : '高优先级审批' }}</span>
          </template>
            ...
            <el-table :data="priorityTasks" class="mini-table" size="small" :show-header="true">
              ...
              <el-table-column prop="risk" label="风险" min-width="80">
                <template #default="{ row }">
                  <span class="ap-risk">{{ row.risk }}</span>
                </template>
              </el-table-column>
...
const apKpis = computed(() => {
  const isGc = role.value.key === 'gc';
  return [
    { value: isGc ? 5 : 8, label: `${isGc ? '待我决策' : '待我审批'} · Demo data` },
    { value: 3, label: '临近SLA · Demo data' },
    { value: 1, label: '已超时 · Demo data' },
    { value: 2, label: '退回待补充 · Demo data' },
    { value: 18, label: '本周已处理 · Demo data' }
  ];
});

/** BU / GC 高优先级审批/治理决策表格数据 */
const priorityTasks = computed<PriorityTask[]>(() => {
  if (role.value.key === 'gc') {
    return [
